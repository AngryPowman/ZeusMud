...
template <typename T> void GameSessionManager::send_message(uint64 guid, uint32 opcode, const T& message)
{
...
template <typename T> void GameSessionManager::send_message(
...
void GameSessionManager::send_error_ex(uint64 guid, uint32 error_code, const std::string& error_reason)
{
    auto iter_session = _sessions.find(guid);
    if (iter_session != _sessions.end())
    {
